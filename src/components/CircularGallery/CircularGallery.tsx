...
class App {
  ...
  createMedias(items?: GalleryItem[], bend: number = 1, textColor?: string, borderRadius: number = 0, font?: string) {
  ...
  addEventListeners() {
    ...
    if (container) {
      container.addEventListener('mousedown', this.boundOnTouchDown);
...
const CircularGallery = forwardRef<CircularGalleryHandle, CircularGalleryProps>(function CircularGallery(
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Circular image gallery. Drag or use left and right arrow keys to navigate."
    />
  );
});
