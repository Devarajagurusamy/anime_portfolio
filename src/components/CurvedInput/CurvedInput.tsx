...
const edgeSeg = (g: Geometry, uTo: number, v: number, ltr: boolean) => {
  ...
  const rho = round2((g.R ?? 0) - (g.dir ?? 1) * v);
  const sweep = ltr === (g.dir ?? 1) > 0 ? 1 : 0;
...
// Safe layout effect hook for SSR Next.js
...
export const CurvedInput: React.FC<CurvedInputProps> = ({
  ...
  const shadow = shadowSize !== 'none' ? SHADOWS[shadowSize] : undefined;
